...
I noticed the secondary and tertiary insurance types were missing for alot of patients, likely indicating that there is no additional coverage for these patients. They were classified into a no additional coverage category below. I also converted the relevant variables into factors and ordered them, if applicable.
```{r data_prep}
...
hospital_df$payment_typology_2 <- as.factor(hospital_df$payment_typology_2)
hospital_df$payment_typology_3 <- as.factor(hospital_df$payment_typology_3)
...
mode_severity <- names(which.max(table(hospital_df$apr_severity_of_illness_description)))
mode_mortality <- names(which.max(table(hospital_df$apr_risk_of_mortality)))

# Replace missing values with the mode
hospital_df$apr_severity_of_illness_description[is.na(hospital_df$apr_severity_of_illness_description)] <- mode_severity
hospital_df$apr_risk_of_mortality[is.na(hospital_df$apr_risk_of_mortality)] <- mode_mortality
...
multinom_model <- multinom(payment_typology_1 ~ age_group + apr_severity_of_illness_description + 
                           ...
                           data = hospital_df)
prop.score <- predict(multinom_model, type = "probs")

hospital_df$ps.logit <- prop.score # the logistic regression estimated PS
...
formula <- payment_typology_1 ~ age_group + apr_severity_of_illness_description + 
           apr_risk_of_mortality + type_of_admission + apr_medical_surgical_description

# Calculate the number of predictor variables
num_predictors <- length(setdiff(names(hospital_df), "payment_typology_1"))
...
tree.num <- seq(100, 300, by=100)  # adjust based on computational feasibility

# Loop through number of trees to optimize model
results.trees <- list()
for (ntree in tree.num) {
  set.seed(123)
  rf.train <- train(formula,
                    data=hospital_df,
                    method="ranger",
